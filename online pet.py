...
while True:
    ...
    if event == 1:
        print("\n🍖 Lucky find! %s found food." % name)
        hunger = min(hunger + 15, 100)
    elif event == 2:
        print("\n🎾 %s found a new toy!" % name)
        happiness = min(happiness + 15, 100)
    elif event == 3:
        print("ur pet had a great poop")
        health = min(health + 15, 0)
    elif event == 4:
        print("\n💤 %s had a great nap." % name)
        health = min(health + 10, 100)
    elif event == 5:
        print("\n🌧️ Bad weather made %s grumpy." % name)
        happiness = max(happiness - 10, 0)
    elif event == 6:
        print("\n✨ Nothing unusual happened today.")
        happiness = min(happiness + 10, 100)
    elif event == 7:
        print("\n🦝 A wild animal stole some food!")
        hunger = max(hunger - 20, 0)
    elif event == 8:
        print("\n💔 %s feels lonely today." % name)
        happiness = max(happiness - 20, 0)
    elif event == 9:
        print("\n🤢 %s ate something bad." % name)
        hunger = max(hunger - 10, 0)
        health = max(health - 20, 0)
    else:
        print("\n⚡ %s got injured while exploring!" % name)
    ...
    while action not in ["feed", "play", "sleep"]:
        print("invalid input, please input a real action")
        action = input("what would you like to do? (feed, play, sleep)").strip().lower()
    if action == "feed":
        print()
        ...
        hunger = max(hunger - 15, 0)
        health = min(health + 5, 1000)
        print()
    ...
    elif happiness <=20 and happiness >= 0:
        health = max(health - 10, 0)
        print()
        print(f"{name} is getting sad! Please play with it! Health decreased to", "x" * (health // 10), f"({health}).")
    health = min(health, 100)
    print()
    end = input("Do you want to continue to the next day? yes/no(quit) ").strip().lower()
    if end == "no":
        ...
        if score <= 500:
            print("You ended the run too early!. Try again!")
        elif score <= 1000:
    ...
    if health <= 0:
        print()
        print(f"{name} has passed away. Game over. you get",str(score),  "points. your pet lasted for {days} days.")
        if score <= 500:
            print("You did not do well!. Try again!")
        elif score <= 1000:
            print("You did okay! Try again to get a better score!")
        elif score <= 2000:
            print("You did well! Try again to get a better score!")
